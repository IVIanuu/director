...
fun Router.doOnChangeStarted(
    ...
    onChangeStarted: (to: Controller?, from: Controller?, isPush: Boolean, container: ViewGroup, handler: ControllerChangeHandler) -> Unit
) = addChangeListener(recursive, onChangeStarted = onChangeStarted)

fun Router.doOnChangeCompleted(
    recursive: Boolean = false,
    onChangeCompleted: (to: Controller?, from: Controller?, isPush: Boolean, container: ViewGroup, handler: ControllerChangeHandler) -> Unit
) = addChangeListener(recursive, onChangeCompleted = onChangeCompleted)
...
fun Router.addChangeListener(
    ...
    onChangeCompleted: ((to: Controller?, from: Controller?, isPush: Boolean, container: ViewGroup, handler: ControllerChangeHandler) -> Unit)? = null
): ControllerChangeListener {
    val listener = object : ControllerChangeListener {
        ...
        override fun onChangeCompleted(
            ...
            onChangeCompleted?.invoke(to, from, isPush, container, handler)
        }
    }
